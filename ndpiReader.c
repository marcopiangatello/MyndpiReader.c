...
static void printFlow(u_int16_t id, struct ndpi_flow_info *flow, u_int16_t thread_id) {
  ...
  if(!json_flag) {
    ...
    fprintf(out, " n.p.h.r.s: %d ", flow->n_pckt_human_readable_string);

    if( (flow->detected_protocol.app_protocol == 7) && (flow->n_pckt_human_readable_string == 0) ) printf("!WARNING!");
    if( (flow->detected_protocol.app_protocol == 5) && (flow->n_pckt_human_readable_string == 0) ) printf("!WARNING!");
    //if( (flow->detected_protocol.app_protocol == 10 ) && (flow->n_pckt_human_readable_string == 0) ) printf("!WARNING!");
    //if( (flow->detected_protocol.app_protocol == 124) && (flow->n_pckt_human_readable_string != 0) ) printf("!WARNING!");
    //if( (flow->detected_protocol.app_protocol == 91) && (flow->n_pckt_human_readable_string > 5 ) ) printf("!WARNING!");
